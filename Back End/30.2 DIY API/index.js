...
app.get("/jokes/:id", (req, res) => {
  ...
  const result = jokes.find((joke) => joke.id === reqId);
  res.json(result);
});
...
app.put("/jokes/:id", (req, res) => {
  ...
  const idx = jokes.findIndex((joke) => joke.id === reqId);
...
app.patch("/jokes/:id", (req, res) => {
  const reqId = parseInt(req.params.id);
  const previousJoke = jokes.find((joke) => joke.id === reqId);
  const replaceJoke = {
    id: reqId,
    jokeText: req.body.text || previousJoke.jokeText,
    jokeType: req.body.type || previousJoke.jokeType,
  };
  const idx = jokes.findIndex((joke) => joke.id === reqId);
  jokes[idx] = replaceJoke;
  res.json(replaceJoke);
...
app.delete("/jokes/:id", (req, res) => {
  ...
  if (idx > -1) {
    ...
    res
      .status(404)
      .json({ error: `Joke with id ${idx} not found. No jokes were deleted` });
  }
});
...
app.delete("/all", (req, res) => {
  ...
  if (userKey == masterKey) {
    ...
    res
      .status(404)
      .json({ error: `Wrong Master Key. No records were deleted` });
  }
});
